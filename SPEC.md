...
- `ANSUT-DSIS/sitewebgrandechancellerie` — reference for **CI, Husky, and the skill family** (but legacy Ferry: 5 per-role workflows @v0.17.0, per-column Jira rules).
- `big-emotion/support-agent-chancellerie` — reference for **router-model Ferry** (v1.1.x) and for skills already de-coupled from website concerns.
...
- Migrating sitewebgrandechancellerie to the router model (separate initiative).
...
2. **Given** `support-agent-chancellerie`, **when** the skill runs gap analysis, **then** it reports: M1 partial (ci.yml lacks lint/format/typecheck/gitleaks), M2 missing, M3/M4/M5/M6 present — and installing M1+M2 yields green CI + working hooks without touching the present modules.
