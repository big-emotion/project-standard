...
- Auditing the repos the standard is installed on (e.g. `the website repo`, `the support-agent repo`) — each consumer repo has its own `<slug>-audit` skill; this audit scores only the plugin repo itself.
